...
import { PriceArrowIcon } from "@/ui-components/icons/price-arrow-icon";
    ...
    () => [
      ...
      columnHelper.accessor((row) => row.name, {
        ...
        cell: (info) => {
          ...
          return (
            <button
              className={cn(styles.button, { [styles.selected]: true })}
              key={row.id}
      ...
      columnHelper.accessor((row) => row.current_price, {
        id: "priceUSD",
        cell: (info) => <span>${formatTokenPrice(info.getValue())}</span>,
        header: () => <span>PRICE</span>,
      }),
      columnHelper.accessor((row) => row.market_cap, {
        id: "marketCap",
        cell: (info) => <span>${convertNumber(info.getValue())}</span>,
        header: () => <span>M CAP</span>,
      }),
      ...
      columnHelper.accessor(
        (row) => row.price_change_percentage_1h_in_currency,
        {
      ...
      columnHelper.accessor(
        (row) => row.price_change_percentage_7d_in_currency,
        {
      ...
      columnHelper.accessor(
        (row) => row.price_change_percentage_30d_in_currency,
        {
      ...
      columnHelper.accessor(
        (row) => row.price_change_percentage_1y_in_currency,
        {
